...
const renderPrBody = (owner: string, name: string, force = false): string => {
  ...
  return [
    ...
    "",
    "openspec-flow needs one Claude credential plus two App-identity secrets on this repository (the App can't write them for you):",
    "",
    "```bash",
    `gh secret set CLAUDE_CODE_OAUTH_TOKEN -R ${slug}  # or ANTHROPIC_API_KEY — set exactly one`,
    `gh secret set OPENSPEC_FLOW_APP_ID -R ${slug} -b "<APP_ID>"`,
    `gh secret set OPENSPEC_FLOW_PRIVATE_KEY -R ${slug} < private-key.pem`,
    "```",
    "",
    "Without an Anthropic credential (`ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN`), the agent can't run. Without the App secrets, the workflow falls back to `GITHUB_TOKEN` which (a) can't push workflow file changes (`.github/workflows/*`) and (b) attributes commits to `github-actions[bot]` rather than the openspec-flow App.",
    "",
