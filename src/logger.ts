...
export const buildLogger = (): Logger => {
  ...
  //
  // pino-http (which Probot wraps) emits `req` on request and
  // `res` + `responseTime` on response — full headers / status /
  // duration, redundant with the `msg` line that already prints
  // `POST /api/github/webhooks 200 - 3ms`. Drop all three for the
  // pretty stream; they're still captured in the JSON file when
  // LOG_PATH is set.
  const prettyOptions = {
    destination: 1,
