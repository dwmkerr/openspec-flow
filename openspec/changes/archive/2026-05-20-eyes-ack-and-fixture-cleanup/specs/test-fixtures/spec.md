...
Contract for identifying and cleaning up GitHub-side artefacts
(issues, PRs, branches) created by `tests/scripts/test-*.sh` smoke
scripts. Ensures every fixture can be located by a single marker
label and wiped by a single command, eliminating orphan accumulation
when scenarios are renamed or scripts removed.
