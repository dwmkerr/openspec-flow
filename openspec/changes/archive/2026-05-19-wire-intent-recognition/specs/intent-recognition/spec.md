...
Deterministic, pure-function classification of GitHub webhook events
into typed openspec-flow intents. The classifier reads only the event
name and payload, returns a discriminated `Intent` union, and never
performs I/O. A separate dispatcher posts a comment on the relevant
issue or PR for actionable and visible-noop intents.
